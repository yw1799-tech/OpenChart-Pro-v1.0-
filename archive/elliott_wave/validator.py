...
class SyntheticWaveGenerator:
    ...
    @staticmethod
    def _interpolate(
        start: float, end: float, n_bars: int, noise_pct: float = 0.0
...
class ValidationReport:
    ...
    def summary(self) -> str:
        lines = [
            ...
            f"  方向准确率: {self.correct_dir_count}/{self.total} = {self.correct_dir_count / self.total:.1%}"
            if self.total > 0
            else "",
        ]
        ...
        if self.predictions:
            ...
            lines.append(
                f"  命中率: {sum(1 for p in self.predictions if p.get('hit'))}"
            ...
            if self.random_baseline > 0:
                ...
                lines.append(f"  随机基线: {self.random_baseline:.1%} | 超越基线: {diff:+.1%}")
...
class ValidationRunner:
    ...
    def _check_detection(
        ...
        if verbose:
            print(f"\n{'=' * 60}")
            print(
                f"诊断: 期望={type_name} 方向={'UP' if expected_dir == Direction.UP else 'DOWN'} 噪声={noise_level:.1%}"
            )
            print(f"{'=' * 60}")
            ...
            for i, p in enumerate(patterns):
                ...
                for w in p.waves:
                    ...
                    print(
                        f"      {w.label}: idx[{w.start.index}->{w.end.index}] "
        ...
        if not patterns:
            if verbose:
                print(f"\n结论: 未检测到任何模式!")
            return ValidationResult(type_name, noise_level, False, False, False, 0.0, "未检测到任何模式")
        ...
        if best_match:
            ...
            return ValidationResult(
                type_name, noise_level, True, True, True, best_conf, f"正确识别: {best_match.summary()}"
            )
        ...
        if verbose:
            ...
            print(
                f"所有检测到的模式类型: {[PATTERN_NAMES_CN.get(p.pattern_type, p.pattern_type.value) for p in patterns]}"
            )
            # 尝试用已知点直接验证规则
        ...
        return ValidationResult(
            type_name, noise_level, True, False, dir_match, top.confidence, f"识别为{top_name}(期望{type_name})"
        )
            ...
            for sample_i in range(n_samples):
                ...
                for direction in [Direction.UP, Direction.DOWN]:
                    ...
                    df, ans = gen.generate_zigzag(
                        base=100,
                        a_pct=np.random.uniform(0.10, 0.20),
    ...
    def test_prediction(
        ...
        for start in range(0, n - min_window - step, step):
            ...
            for window_size in window_sizes:
                ...
                self.analyzer.analyze(sub_df, high_col=high_col, low_col=low_col, close_col=close_col)
            ...
            for name, target in best_pred.target_prices.items():
                if target > 0:
                    ...
                    if (
                        abs(future_high - target) / target <= tolerance
                        or abs(future_low - target) / target <= tolerance
                        or (future_low <= target <= future_high)
                    ):
                        hit = True
                        hit_target = name
                        break
...
def test_rules_directly() -> str:
    ...
    from .rules import (
        ...
        check_flat_expanded,
        check_flat_running,
        check_triangle_contracting,
        check_triangle_expanding,
        check_leading_diagonal,
        check_ending_diagonal,
        check_truncated_5th,
        check_double_zigzag,
        check_combination_wxy,
        classify_impulse_extension,
    )
    ...
    def _test(name, rule_fn, swings, labels, direction, expected_valid=True):
        ...
        if ok:
            ...
            failed += 1
        hard_detail = " ".join(f"{r.name}={'Y' if r.passed else 'N'}" for r in results if r.is_hard)
    ...
    sz = [Swing(0, 120, True), Swing(20, 100, False), Swing(40, 110, True), Swing(60, 85, False)]
    _test("锯齿形DOWN", check_zigzag, sz, ["A", "B", "C"], Direction.DOWN, True)

    # UP zigzag: A up, B down, C up
    szu = [Swing(0, 100, False), Swing(20, 120, True), Swing(40, 110, False), Swing(60, 135, True)]
    _test("锯齿形UP", check_zigzag, szu, ["A", "B", "C"], Direction.UP, True)

    # 违反: B超A起点
    szf = [Swing(0, 120, True), Swing(20, 100, False), Swing(40, 125, True), Swing(60, 85, False)]
    _test("B超A起点(应失败)", check_zigzag, szf, ["A", "B", "C"], Direction.DOWN, False)

    # ========== 规则平台 ==========
    lines.append("\n--- 规则平台 ---")
    sf = [Swing(0, 120, True), Swing(20, 100, False), Swing(40, 118, True), Swing(60, 95, False)]
    _test("规则平台DOWN", check_flat_regular, sf, ["A", "B", "C"], Direction.DOWN, True)

    # ========== 扩展平台 ==========
    lines.append("\n--- 扩展平台 ---")
    sfe = [Swing(0, 120, True), Swing(20, 100, False), Swing(40, 125, True), Swing(60, 90, False)]
    _test("扩展平台DOWN(B超A起点)", check_flat_expanded, sfe, ["A", "B", "C"], Direction.DOWN, True)

    # ========== 顺势平台 ==========
    lines.append("\n--- 顺势平台 ---")
    sfr = [Swing(0, 120, True), Swing(20, 100, False), Swing(40, 125, True), Swing(60, 105, False)]
    ...
    _test("扩展三角DOWN", check_triangle_expanding, ste, ["A", "B", "C", "D", "E"], Direction.DOWN, True)
    ...
    _test(
        "扩展三角-浪不放大(应失败)", check_triangle_expanding, ste_f, ["A", "B", "C", "D", "E"], Direction.DOWN, False
    )
    ...
    _test("双锯齿UP", check_double_zigzag, sdz, ["1", "2", "3", "4", "5"], Direction.UP, True)
    ...
    _test("双重组合UP", check_combination_wxy, scw, ["1", "2", "3", "4", "5"], Direction.UP, True)
    ...
    _test("引导楔形DOWN(浪4入浪1)", check_leading_diagonal, sld, ["1", "2", "3", "4", "5"], Direction.DOWN, True)
    ...
    _test("终结楔形DOWN(浪4必须入浪1)", check_ending_diagonal, sed, ["1", "2", "3", "4", "5"], Direction.DOWN, True)
    ...
    _test("收缩三角DOWN(方向)", check_triangle_contracting, strd, ["A", "B", "C", "D", "E"], Direction.DOWN, True)
    ...
    _test("浪4等于浪1终点(严格>判定,应失败)", check_impulse, sb_eq, ["1", "2", "3", "4", "5"], Direction.UP, False)
    ...
    sb_near = [
        ...
        Swing(40, 110, False),
        Swing(60, 142, True),
        Swing(80, 120.01, False),
        Swing(100, 150, True),
    ]
    _test("浪4略高于浪1终点(边界附近应通过)", check_impulse, sb_near, ["1", "2", "3", "4", "5"], Direction.UP, True)

    # ========== 延伸分类 ==========
    lines.append("\n--- 延伸分类 ---")
    waves_ext3 = [
        Wave(Swing(0, 100, False), Swing(20, 120, True), "1"),
    ...
    if data_path:
        try:
            ...
            if len(col_map) >= 3:
                ...
                # 审计#19: 计算随机基线命中率作为对比
                random.seed(42)
                report_pred.random_baseline = runner._calc_random_baseline(df, n_trials=200, close_col=col_map["close"])
                ...
                if report_pred.predictions:
                    ...
                    lines.append(
                        f"    随机基线: {report_pred.random_baseline:.1%} | "
