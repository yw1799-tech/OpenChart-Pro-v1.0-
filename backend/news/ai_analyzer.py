...
- **confirm (ai_confidence ≥ 65)**: 方向与主趋势一致 + 主要风险可控 → 值得跟单
  ...
  * 横盘震荡市(ADX < 25): 距阻力 0.3-1% 视为可控风险, 配合其他正向因素仍可 confirm
- **warn (40-64)**: 信号成立但有具体可量化风险 → 减仓跟或观察
  * 例: 多头趋势但 RSI=72 已超买 → BUY warn
  * 例: 距关键阻力 0.3-1% (空间偏小但不是绝望) → warn
  * 例: 单一指标不利 (如缩量 / OI 微降) 但其他维度支持 → warn
- **reject (< 40)**: 技术面方向矛盾 / 新闻明确推翻 / 关键位严重不利 → 放弃
  * 信号方向与主趋势反向(多头排列里出 SELL、空头排列里出 BUY)
  * 距关键阻力/支撑 < 0.3% 几乎无空间 (v12.27.16: 0.5%→0.3%, 加密横盘市更宽松)
  * 明确利空新闻 + 技术面假突破

⚠️ **关于"空头回补"标签的克制**:
- 单纯 "OI 下降 + 价升 → 空头回补" **不应作为 reject 唯一理由**
- 必须叠加其他证据 (如缩量 + DI- 主导 + 距阻力 < 0.3%) 才升级到 reject
- 否则: 横盘震荡市中几乎所有反弹都会被打成"空头回补", 系统永远不开仓
