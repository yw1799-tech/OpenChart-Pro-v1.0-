...
async def _pool_fundamentals_refresh_loop():
    ...
    while True:
        ...
        try:
            ...
            # v12.20.14: limit 500 → 2000 (修复"低分股基本面长期不刷新"bug)
...
async def _refresh_pool_symbols_cache():
    ...
    while True:
        try:
            # v12.20.14: limit 500 → 2000 (符号缓存覆盖全候选池, 信号匹配不漏低分股)
            items = await db.get_pool_items(status=None, limit=2000)
...
async def list_pool_items(
    ...
    """查询候选池条目，按评分降序。
    v12.20.14: limit 上限 500 → 2000 (候选池总规模 1450, 单市场最大 650 + buffer)
    """
...
async def cleanup_pool(dry_run: bool = Query(False, description="True 只返回将被清理的列表，不实际删除")):
    ...
    # v12.20.14: limit 500 → 2000 (清理覆盖全候选池, 不漏低分股)
    items = await db.get_pool_items(limit=2000)
    removed = []
    kept = 0
    for it in items:
