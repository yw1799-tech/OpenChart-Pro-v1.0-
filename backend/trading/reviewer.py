...
REVIEW_PROMPT = """你是一位资深交易复盘分析师。任务是对一笔已闭环交易做客观全链路复盘,识别每个环节的决策质量与改进空间。

【分析原则】
1. **只评决策当时可知的信息**: 不能用事后才知道的高低点指责入场时机;只能基于"决策那一刻该有什么信息"评价。
2. **决策对 ≠ 结果好**: 一笔好决策可能因为运气亏钱(decision_score 高 / outcome_score 低);一笔烂决策可能撞运气赚钱(反之)。两者要分开评分。
3. **客观评分,避免一律给 C**: 决策合理就给高分,合理就 verdict=good;模糊不要默认 neutral。
4. **数据缺失的环节直接标 verdict=neutral + summary 注明**: 不要在缺失数据上推断。
...
**A. 9 环节逐项评估**: 每个环节给 score (0-100) + verdict + 80 字摘要 + 至少 1 条具体教训(type 必须用环节名)。每个 summary 必须重申"只评决策当时可知信息"。

**B. 根因定位**: 找出影响 PnL 的关键单一环节(可能是信号、AI 验证、SL/TP 设置、出场或仓位管理)。

**C. "如果做对"沙盘**: 假设关键环节做对,结果改善多少%(具体数字 + 哪个环节)。例:
- "若出场提前到 period_high(基于 RSI > 75 退出规则,决策时可见),本笔可达 +15%"
- "若信号 ai_confidence < 70 时拒绝入场(基于历史胜率统计),可避免 -5% 亏损"
...
{{
  "score": 0-100 综合评分,
  "grade": "A|B|C|D",
  "decision_score": 0-100 (仅看决策合理性,不看结果),
  "outcome_score": 0-100 (仅看实际结果),
  "pros": ["决策合理之处 2-3 条"],
  "cons": ["决策不合理之处 2-3 条"],
  "primary_lesson": "最重要的教训(一句话,<= 50 字,基于决策当时可知信息)",
  "what_if_better": "若某环节做对,结果改善多少%(具体数字 + 哪个环节)",

  "entry_analysis": "入场综合分析 80 字内(策略+时机+市场上下文,基于决策时信息)",
  "mid_analysis":   "持仓中综合分析 80 字内(加减仓/诊断响应/风险管理)",
  "exit_analysis":  "出场综合分析 80 字内(触发合理性/是否过早过晚/错失利润)",

  "link_evaluations": {{
    "signal":      {{"score": 0-100, "verdict": "good|bad|neutral", "summary": "策略信号本身是否合理(多周期一致性、置信度合理性)?基于决策当时可知信息评估。(80 字内)", "lessons": [{{"type":"signal","content":"..."}}]}},
    "ai_verify":   {{"score": 0-100, "verdict": "...", "summary": "AI 验证判断的合理性(基于当时数据,而非事后)。(80 字内)", "lessons": [...]}},
    "news_judge":  {{"score": 0-100, "verdict": "...", "summary": "入场新闻判断的合理性(是否准确反映了当时已发布的新闻)。(80 字内)", "lessons": [...]}},
    "entry_timing":{{"score": 0-100, "verdict": "...", "summary": "入场时机(基于当时技术面,而非事后高低点)。(80 字内)", "lessons": [...]}},
    "sl_tp_setup": {{"score": 0-100, "verdict": "...", "summary": "AI 给的 SL/TP 是否基于支撑阻力(决策时可计算)。(80 字内)", "lessons": [...]}},
    "mid_management":{{"score": 0-100, "verdict": "...", "summary": "持仓中加减仓决策(基于持仓时点信息,而非事后)。(80 字内)", "lessons": [...]}},
    "diagnose_response":{{"score": 0-100, "verdict": "...", "summary": "AI 诊断变化的响应及时性(基于诊断发布时点)。(80 字内)", "lessons": [...]}},
    "exit_quality":{{"score": 0-100, "verdict": "...", "summary": "出场触发的合理性(基于触发时刻规则,而非事后高点)。(80 字内)", "lessons": [...]}},
    "money_management":{{"score": 0-100, "verdict": "...", "summary": "仓位大小/盈亏比合理性(基于决策时账户状态)。(80 字内)", "lessons": [...]}}
  }},

  "improvements": "200 字: 3 条针对薄弱环节的具体可执行改进(如'下次同策略信号 ai_confidence < 75 时仓位降到 50%')",
...
每个 link_evaluations.lessons 至少 1 条,type 必须用 9 个环节名之一(不要用 entry/exit/risk 旧标签)。
只输出 JSON,不要 markdown 包围。
"""


WEEKLY_PROMPT = """你是一位资深交易复盘教练。任务是基于本周所有交易数据,给出客观的周度评估和可量化的改进方向。

【分析原则】
1. 用数据说话: 引用 Top 赚/亏样本、胜率、各市场分布等具体统计;不写"表现尚可"这类含糊评价。
2. recurring_mistakes 限 3 条以内,按严重度排序(避免稀释重点)。
3. actionable_changes 必须可执行: 含数值阈值/策略名/市场名,不写"加强风控"这种空话。
...
{{
  "summary": "200-300 字综合评估,引用具体统计数据(如'本周胜率 60%,Top 1 赚 $300 来自 XX 策略,Top 1 亏 $150 来自 RSI 超买追入')说明: 整体表现 / 关键亮点 / 主要问题 / 趋势变化",
  "recurring_mistakes": [
    "[严重] 具体错误模式 1(如'连续 3 次 RSI > 80 追入全部亏损,共亏 $X')",
    "[中度] 错误模式 2"
  ],
  "actionable_changes": [
    "下周可执行动作 1(含具体数值/策略名/市场,如'XX 策略胜率仅 30%,下周仅在 ADX > 25 时启用')",
    "动作 2",
    "动作 3"
  ]
}}
...
【约束】
- recurring_mistakes 最多 3 条,按 severity (严重→中度→轻微) 降序;无明显模式时返回空数组,不要硬凑
- actionable_changes 最多 3 条,每条必须可执行(含数值/策略名/市场名);避免"加强风控"这种空话
- summary 必须引用本周具体统计数据(胜率/Top 样本/总盈亏),不写"表现尚可"这种含糊评价
- 数据不充分时(如本周仅 1-2 笔交易),summary 直接说"样本量不足,本周仅 N 笔,统计意义有限",mistakes/changes 可空数组
...
class TradeReviewer:
    ...
    async def _llm_swap_deep_review(self, **kw) -> Optional[Dict]:
        ...
        prompt = f"""你是一位资深加密永续合约复盘分析师。任务是对一笔已闭环合约交易做客观复盘,聚焦合约特有的杠杆、funding、强平等风险维度。

【分析原则】
1. 只评决策当时可知的信息(不能用事后高低点指责)
2. 决策对 ≠ 结果好: 两者分开评分(decision_score / outcome_score)
3. 数据缺失维度直接标 verdict=neutral,不在缺失数据上推断
4. 合约杠杆放大风险,关键关注: 杠杆合理性、SL 抗噪、funding 累积、强平风险
...
{{
  ...
  "entry_analysis": "入场综合分析 80 字内(杠杆/时机/方向是否合理,基于决策时信息)",
  "mid_analysis":   "持仓中综合分析 80 字内(BE/trailing/funding 累积管理)",
  "exit_analysis":  "出场综合分析 80 字内(SL/TP 触发或强平,主动 vs 被动)",

  "link_evaluations": {{
    "leverage_choice": {{"score": 0-100, "verdict": "good|bad|neutral", "summary": "杠杆选择(基于入场时 ATR 和账户规模)是否合理。强平距离 < 5% 视为过高风险。(60 字内)"}},
    "entry_timing":    {{"score": 0-100, "verdict": "...", "summary": "入场时机(基于当时技术面+资金费率,非事后高低点)。(60 字内)"}},
    "sl_setup":        {{"score": 0-100, "verdict": "...", "summary": "SL 是否抗噪(浮亏 < 2% 即触视为过紧;> 8% 视为过松)。(60 字内)"}},
    "tp_trailing":     {{"score": 0-100, "verdict": "...", "summary": "TP/trailing 触发合理性。(60 字内)"}},
    "funding_cost":    {{"score": 0-100, "verdict": "...", "summary": "长期持仓 funding 累积是否合理(累积 funding 占 PnL 比例)。(60 字内)"}},
    "exit_quality":    {{"score": 0-100, "verdict": "...", "summary": "出场触发合理性(主动 vs 被动强平)。(60 字内)"}}
  }},

  "turning_points": [{{"time": "MM-DD HH:MM", "event": "...", "impact": "..."}}],
  "improvements": "本次具体可执行改进(含数值阈值,如'下次同方向首仓杠杆从 5x 降到 3x')",
  "lessons": [{{"type": "leverage_choice|entry_timing|sl_setup|tp_trailing|funding_cost|exit_quality", "content": "可复用教训(含数值阈值优先)"}}],
  "primary_lesson": "一句话核心教训(<= 50 字)",
  "what_if_better": "若做对一件事,净 PnL 改善多少%(具体数字 + 哪个环节)"
}}

合约特别关注(评分时考虑):
- 杠杆 vs ATR 的匹配度(高 ATR 不应高杠杆)
- SL 与 ATR 的关系(SL 距离应 ≥ 1.5×ATR 抗噪)
- funding 长期累积是否抵消 PnL
- 强平触发说明仓位/杠杆设置严重失误,exit_quality 应给低分
...
"""
    ...
    async def _llm_strategy_param_analysis(
        ...
        prompt = f"""你是一位资深量化策略调参分析师。任务是基于一笔已闭环交易,客观评价该笔交易中各策略的表现质量,而不是评价"参数本身"——单笔样本不足以推断参数本身的统计有效性。

【分析原则】
1. **单笔样本只能评"该笔表现"**,不能评"参数本身好坏"(那需要 N≥30 笔同策略统计)
2. 若该笔表现差,标 evaluation=2-3 + 注明"需累积更多样本评估参数",**不要直接建议改参数**(suggested_params 应为 null)
3. 若该笔表现明显异常(如参数与市场风格严重错配,从 K 线即可看出),才建议改参数 + 必须给出**具体数值键名**
4. 数据缺失时不在缺失上推断
...
信号置信度: {sig['confidence'] or 0}; AI 验证置信度: {sig['ai_confidence'] or 0}
...
【评估维度】
对每个策略评估:
1. 该笔交易中,该策略入场时机的合理性(★1-5)
2. 当前参数与市场风格(如 {market} 趋势/震荡)是否匹配,从入场时市场快照判断
3. **suggested_params 仅在以下情况填**: (a) 该笔表现明显由参数错配导致 (b) 你能给出具体合法参数键名(必须是该策略已有参数的同名键)
4. 若不确定参数应改成什么,suggested_params=null,改进建议放在 expected_improvement 文字描述中
...
{{
  "strategies": [
    {{
      "name": "策略名",
      "current_params": {{原参数键值原样回填}},
      "evaluation": 1-5,
      "reason": "<= 80 字理由(说明评估依据是该笔表现,非参数本身)",
      "suggested_params": {{修改后的参数;键名必须与 current_params 一致}} 或 null,
      "expected_improvement": "<= 50 字(说明改动预期效果);若 suggested_params=null 可写'需累积 N 笔后再评估'"
    }}
  ],
  "overall_conclusion": "<= 60 字综合结论(说明本次评估限于单笔样本)"
}}

【硬性约束】
- suggested_params 的键必须与 current_params 完全相同(只能改值,不能加新键),否则后端无法解析
- 单笔评分通常给 2-4 之间;1 或 5 极端值需在 reason 给出明确证据
- 不要因为该笔亏损就草率给低分;亏损可能是市场原因而非参数原因
...
"""
    ...
    async def _llm_translate_lesson(self, lesson_text: str, pool_id: str,
        ...
        if not self.ai_analyzer or not lesson_text:
            return None
        prompt = f"""你是交易系统风控规则编译器。任务是把一条复盘教训转换成系统可执行的硬规则,严格遵守规则类型枚举,不可参数化的教训诚实返回 null。

【教训文本】
{lesson_text}

【可选 rule_type】(封闭枚举,不能创造新类型)
...
5. null                 — 该教训不适合编译为硬规则(无可量化参数)

【输出严格 JSON】
{{"rule_type": "rsi_block 或其他 4 种之一 或 null", "params": {{...}}}}

【参数兜底默认值】(教训文本未给具体阈值时使用)
- 教训含 "RSI" 但无阈值: max_rsi=80, min_dist_to_high_pct=3
- 教训含 "亏损/减仓" 但无阈值: loss_threshold_pct=8, action='reduce_50'
- 教训含 "下跌/趋势" 但无阈值: lookback_h=24, max_drop_pct=-5

【正反例】
正例 1: 教训 "RSI > 80 时不应入场" → {{"rule_type": "rsi_block", "params": {{"max_rsi": 80, "min_dist_to_high_pct": 3}}}}
正例 2: 教训 "亏损 10% 应立即减仓 50%" → {{"rule_type": "drawdown_force_close", "params": {{"loss_threshold_pct": 10, "action": "reduce_50"}}}}
反例: 教训 "应加强情绪管理" → {{"rule_type": null, "params": {{}}}}  (无法量化,诚实返回 null)
反例: 教训 "需关注资金费率" → {{"rule_type": null, "params": {{}}}}  (无可执行硬规则)

只输出 JSON,不要 markdown 包围,不输出其它任何内容。
"""
        try:
    ...
    async def _cluster_lessons_via_llm(self, lessons: List[Dict],
        ...
        prompt = f"""你是一位资深复盘语义聚类分析师。任务是把 {len(lessons)} 条同池/同环节({pool_id} / {lesson_type})的复盘教训按 actionable 改进语义聚类合并,减少教训库重复。

【合并标准】
- 表达同一个核心 actionable 改进(如"AI 验证应加 RSI 检查") = 合并,即便措辞不同
- 数值阈值略有差异(RSI > 80 vs RSI > 85) = 合并,canonical_text 取较严格的阈值
- 不同的具体改进("加 RSI" vs "加 ATR 止损") = 不合并(各自独立 cluster)
- 单条独立成 cluster 也允许
- 不要为了减少 cluster 数量强行合并语义不同的教训

【教训列表】
{lessons_text}

【输出严格 JSON】(必须是顶层 object,clusters 数组在 clusters 字段下)

{{
  "clusters": [
    {{"cluster_id": 1, "lesson_ids": [1707, 1715, 1718], "canonical_text": "AI 验证应加 RSI 超买检查(RSI > 80 拒绝入场)"}},
    {{"cluster_id": 2, "lesson_ids": [1709], "canonical_text": "..."}}
  ]
}}

【硬性约束】
- 顶层必须是 {{"clusters": [...]}} 格式;不要直接返回数组
- lesson_ids 必须是教训文本前的实际 id 数字(不是行号)
- 每个 lesson_id 只能出现在一个 cluster 中(不重复)
- canonical_text 选最具体可执行的版本(含数值阈值优先),≤ 50 字
- cluster_id 从 1 开始连续编号
