...
def create_figure(
    ...
    for lv in lv_lst:
        if plot_macd[lv]:
            ...
            sub_pic_cnt += 1
    figure, axes = plt.subplots(sub_pic_cnt, 1, figsize=(w, total_h), gridspec_kw={"height_ratios": gridspec_kw})
    try:
        axes[0]
...
class CPlotDriver:
    def __init__(self, chan: CChan, plot_config: Union[str, dict, list] = "", plot_para=None):
        ...
        plot_macd: Dict[KL_TYPE, bool] = {
            kl_type: conf.get("plot_macd", False) for kl_type, conf in plot_config.items()
        }
        ...
        for kl in meta.klu_iter():
            ...
            if plot_mode == "kl":
                ...
                raise CChanException(
                    f"unknow plot mode={plot_mode}, must be one of kl/close/open/high/low", ErrCode.PLOT_ERR
                )
        if _x:
            ax.plot(_x, _y)
    ...
    def draw_seg(
        ...
        for seg_idx, seg_meta in enumerate(meta.seg_list):
            ...
            if seg_meta.is_sure:
                ax.plot(
                    [seg_meta.begin_x, seg_meta.end_x], [seg_meta.begin_y, seg_meta.end_y], color=color, linewidth=width
                )
            else:
                ax.plot(
            ...
            if show_num and seg_meta.begin_x >= x_begin:
                ax.text(
                    (seg_meta.begin_x + seg_meta.end_x) / 2,
                    (seg_meta.begin_y + seg_meta.end_y) / 2,
                    f"{seg_meta.idx}",
                    fontsize=num_fontsize,
                    color=num_color,
                )
    ...
    def draw_channel(
        self, meta: CChanPlotMeta, ax: Axes, T=None, top_color="r", bottom_color="b", linewidth=3, linestyle="solid"
    ):
        ...
        if T is None:
            ...
            raise CChanException(
                f"plot channel of T={T} is not setted in CChanConfig.trend_metrics = {config_T_lst}", ErrCode.PLOT_ERR
            )
    ...
    def draw_demark_begin_line(
        self, ax, begin_line_color, plot_begin_set: set, linestyle: str, demark_idx: T_DEMARK_INDEX
    ):
        if (
