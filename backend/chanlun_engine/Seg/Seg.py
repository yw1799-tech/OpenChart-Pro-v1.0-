...
class CSeg(Generic[LINE_TYPE]):
    ...
    def check(self):
        ...
        if self.end_bi.idx - self.start_bi.idx < 2:
            raise CChanException(
                f"线段({self.start_bi.idx}-{self.end_bi.idx})长度不能小于2! idx={self.idx}", ErrCode.SEG_LEN_ERR
            )
    ...
    def cal_macd_metric(self, macd_algo, is_reverse):
        if macd_algo == MACD_ALGO.SLOPE:
            ...
            raise CChanException(
                f"unsupport macd_algo={macd_algo} of Seg, should be one of slope/amp", ErrCode.PARA_ERROR
            )
