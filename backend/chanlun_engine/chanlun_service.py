...
def _build_kline_units(candles: List[Dict[str, Any]]) -> List[CKLine_Unit]:
    """将原始K线数据转换为 CKLine_Unit 列表。跳过脏数据防止 chan.py 全局崩溃。"""
    units = []
    skipped = 0
    for c in candles:
        ts = c.get("timestamp") or c.get("time") or c.get("t", 0)
        o = float(c.get("open") or c.get("o", 0))
        ...
        # 过滤脏 K 线：任一价格 ≤0 / high<low / 价格范围异常（low 是 open 的 < 10% — 可能是 open=1 那种占位）
        if o <= 0 or h <= 0 or l <= 0 or close <= 0:
            skipped += 1; continue
        if h < l:
            skipped += 1; continue
        # 极端差价防护：最大值/最小值比 > 100（K 线内价格差 100x，基本不可能）
        if h / max(l, 1e-9) > 100:
            skipped += 1; continue
        kl_dict = {
    ...
    if skipped:
        logger.warning(f"[chanlun] 跳过 {skipped} 条脏 K 线（价格≤0/high<low/极端差价）")
    return units
...
def analyze(candles: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    # 防御：去重 + 按时间戳升序排序
    # chan.py 要求 K 线严格按时间递增；前端 getDataList 在 loadMore 后可能乱序 / 重复
    seen_ts = set()
    clean_candles = []
    for c in candles:
        ts = int(c.get("timestamp") or c.get("time") or c.get("t", 0))
        if ts <= 0 or ts in seen_ts:
            continue
        seen_ts.add(ts)
        clean_candles.append(c)
    clean_candles.sort(key=lambda x: int(x.get("timestamp") or x.get("time") or x.get("t", 0)))
    if len(clean_candles) != len(candles):
        logger.info(f"[chanlun] 去重 + 排序：原始 {len(candles)} → 处理后 {len(clean_candles)}")
