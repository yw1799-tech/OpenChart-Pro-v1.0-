...
# Gate 1: 位置门
# v12.24.3 Phase 0 校准: 0.05 → 0.03
#   30 天回测显示 5% 拒掉 24/42 = 57% 样本 (拒掉的均 PnL 0.11% 还是正);
#   3% 折中: 拒得少但仍过滤"过高位追涨末段". 看实战数据再调.
DIST_TO_HIGH_MIN_PCT = 0.03

# Gate 2: 已涨幅度门
# v12.24.3 保持 0.04: 30 天仅触发 4/42 = 10% 笔, 阈值合理
INTRADAY_PUMP_MAX_PCT = 0.04

# Gate 3: 风险回报门
# v12.23.1 审计 P1: 1.5 → 3.0 (旧 SL 抗噪 ≥ 2.5% + TP 25% 默认 → R:R = 10:1, 1.5 形同虚设)
# v12.24.3 Phase 0 校准: 3.0 → 2.0
#   30 天回测发现:
#     - R:R < 3.0 拒掉 25 笔 但贡献 +12.79% (这些笔均 PnL 0.51%, 量大)
#     - R:R ≥ 5.0 仅 2 笔且全亏 (-2.05%) → AI 给的高 R:R 不可靠
#     - 缺 SL/TP 的 15 笔 (缠论) 反而最强 +25.84% — 不动这条 (skipped 路径已保留)
#   2.0 是数据驱动的折中: 比 3.0 留更多 PnL 贡献仓, 又比 1.5 (旧值) 实质过滤
MIN_RISK_REWARD = 2.0

# Gate 4: 大盘环境门
# v12.23.1 审计 P1: -1.5% → -2.0% (SPY 日内 1% 波动家常便饭, 1.5% 阈值偏严)
# v12.24.3 保持 -2.0%: 30 天 SPY 没跌过 -2%, 触发 0 次, 数据样本不足无法校准
