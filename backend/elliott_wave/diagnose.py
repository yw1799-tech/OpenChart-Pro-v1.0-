...
from App.elliott_wave.core import (
    PatternType, Direction, PATTERN_NAMES_CN, Swing, Wave,
    compute_confidence, MOTIVE_TYPES, CORRECTIVE_TYPES
)
...
def diagnose_single(name: str, df, answer: dict, analyzer: ElliottWaveAnalyzer):
    ...
    if correct_rank:
        ...
        if correct_rank > 1:
            print(f"  排名第1的是: {PATTERN_NAMES_CN.get(patterns[0].pattern_type, '?')} conf={patterns[0].confidence:.1%}")
    elif len(patterns) > 0:
...
def run_diagnosis():
    ...
    df, ans = gen.generate_impulse(
        base=100, w1_pct=0.20, w2_retrace=0.50, w3_ext=1.618,
        w4_retrace=0.382, w5_ratio=1.0,
        direction=Direction.UP, bars_per_wave=40, noise_pct=noise
    )
    ...
    df, ans = gen.generate_impulse(
        base=100, w1_pct=0.20, w2_retrace=0.50, w3_ext=1.618,
        w4_retrace=0.382, w5_ratio=1.0,
        direction=Direction.DOWN, bars_per_wave=40, noise_pct=noise
    )
    results["推动浪DOWN"] = diagnose_single("推动浪DOWN", df, ans, analyzer)

    # 3. 锯齿形 DOWN
    df, ans = gen.generate_zigzag(
        base=100, a_pct=0.15, b_retrace=0.50, c_ratio=1.0,
        direction=Direction.DOWN, bars_per_wave=30, noise_pct=noise
    )
    ...
    df, ans = gen.generate_zigzag(
        base=100, a_pct=0.15, b_retrace=0.50, c_ratio=1.0,
        direction=Direction.UP, bars_per_wave=30, noise_pct=noise
    )
    results["锯齿形UP"] = diagnose_single("锯齿形UP", df, ans, analyzer)

    # 5. 规则平台
    df, ans = gen.generate_flat(
        base=100, a_pct=0.20, b_retrace=0.90, c_ratio=1.0,
        flat_type="regular", direction=Direction.DOWN, bars_per_wave=30, noise_pct=noise
    )
    results["规则平台DOWN"] = diagnose_single("规则平台DOWN", df, ans, analyzer)

    # 6. 扩展平台
    df, ans = gen.generate_flat(
        base=100, flat_type="expanded",
        direction=Direction.DOWN, bars_per_wave=30, noise_pct=noise
    )
    ...
    df, ans = gen.generate_triangle(
        ...
        direction=Direction.DOWN, bars_per_wave=25, noise_pct=noise
    )
    ...
    df, ans = gen.generate_triangle(
        base=100, amplitude=0.12, shrink_ratio=0.7,
        direction=Direction.UP, bars_per_wave=25, noise_pct=noise
    )
    ...
    df, ans = gen.generate_impulse(
        base=100, w1_pct=0.20, w2_retrace=0.50, w3_ext=1.618,
        w4_retrace=0.382, w5_ratio=1.0,
        direction=Direction.UP, bars_per_wave=40, noise_pct=0.0
    )
    ...
    df, ans = gen.generate_zigzag(
        base=100, a_pct=0.15, b_retrace=0.50, c_ratio=1.0,
        direction=Direction.DOWN, bars_per_wave=30, noise_pct=0.0
    )
    ...
    df, ans = gen.generate_triangle(
        base=100, amplitude=0.12, shrink_ratio=0.7,
        direction=Direction.DOWN, bars_per_wave=25, noise_pct=0.0
    )
