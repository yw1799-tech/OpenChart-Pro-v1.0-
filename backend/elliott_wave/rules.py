...
def _check_convergence(w1: Wave, w2: Wave, w3: Wave, w4: Wave, w5: Wave,
                        d: int) -> RuleResult:
    """
    审计#1#2修复: 楔形收敛检验
    ...
    detail = (f"action步进: {dp_13:.2f}→{dp_35:.2f} "
              f"gap: {gap_12:.2f}→{gap_34:.2f}")

    return RuleResult("趋势线收敛", converging, True,
                      1.0 if converging else 0.0, detail)
...
def check_impulse(waves: List[Wave], direction: Direction) -> List[RuleResult]:
    ...
    results.append(RuleResult("浪2不超浪1起点", hr1, True, 1.0 if hr1 else 0.0,
                              f"W2端={w2.end.price:.2f} W1始={w1.start.price:.2f}"))
    ...
    results.append(RuleResult("浪3非最短", hr2, True, 1.0 if hr2 else 0.0,
                              f"W1={l1:.2f} W3={l3:.2f} W5={l5:.2f}"))

    hr3 = _above(w4.end.price, w1.end.price, d)
    results.append(RuleResult("浪4不入浪1区域", hr3, True, 1.0 if hr3 else 0.0,
                              f"W4端={w4.end.price:.2f} W1端={w1.end.price:.2f}"))

    hr4 = _above(w3.end.price, w1.end.price, d)
    results.append(RuleResult("浪3超越浪1", hr4, True, 1.0 if hr4 else 0.0,
                              f"W3端={w3.end.price:.2f} W1端={w1.end.price:.2f}"))

    hr5 = _above(w5.end.price, w3.end.price, d)
    results.append(RuleResult("浪5超越浪3", hr5, True, 1.0 if hr5 else 0.0,
                              f"W5端={w5.end.price:.2f} W3端={w3.end.price:.2f}"))

    # --- 软规则 ---
    r21 = _ratio(w1, w2)
    results.append(RuleResult("浪2回撤比", True, False,
                              fib_score(r21, [0.382, 0.500, 0.618], 0.08),
                              f"W2/W1={r21:.3f}"))

    r31 = _ratio(w1, w3)
    results.append(RuleResult("浪3扩展比", True, False,
                              fib_score(r31, [1.0, 1.272, 1.618, 2.618], 0.1),
                              f"W3/W1={r31:.3f}"))

    r43 = _ratio(w3, w4)
    results.append(RuleResult("浪4回撤比", True, False,
                              fib_score(r43, [0.236, 0.382, 0.500], 0.08),
                              f"W4/W3={r43:.3f}"))

    r51 = _ratio(w1, w5)
    results.append(RuleResult("浪5比率", True, False,
                              fib_score(r51, [0.618, 1.0, 1.618], 0.1),
                              f"W5/W1={r51:.3f}"))

    diff_24 = abs(r21 - r43)
    results.append(RuleResult("交替原则", True, False,
                              min(1.0, diff_24 / 0.2),
                              f"W2回撤={r21:.3f} W4回撤={r43:.3f} 差={diff_24:.3f}"))

    min_l = min(l1, l3, l5) if min(l1, l3, l5) > 0 else 1
    has_ext = max(l1, l3, l5) >= 1.618 * min_l
    results.append(RuleResult("有延伸浪", True, False, 1.0 if has_ext else 0.3,
                              f"最长/最短={max(l1,l3,l5)/min_l:.2f}"))

    results.append(RuleResult("浪2不过深", True, False,
                              1.0 if r21 <= 0.786 else max(0, 1.0 - (r21 - 0.786) / 0.2),
                              f"W2回撤={r21:.1%}"))
    ...
    if dt_13 > 0 and dt_24 > 0:
        ...
        if abs(slope_13) > 0:
            ...
            parallel = 0.5
        results.append(RuleResult("波浪通道平行", True, False, parallel,
                                  f"斜率13={slope_13:.4f} 斜率24={slope_24:.4f}"))

    # 时间比率: 浪2与浪4的时间应有Fibonacci关系
    d2, d4 = w2.duration, w4.duration
    if d2 > 0 and d4 > 0:
        time_r = d4 / d2
        results.append(RuleResult("浪2/4时间比", True, False,
...
def check_truncated_5th(waves: List[Wave], direction: Direction) -> List[RuleResult]:
    ...
    results.append(RuleResult("浪5未超浪3(失败)", hr5_truncated, True,
                              1.0 if hr5_truncated else 0.0))
    ...
    r51 = _ratio(w1, w5)
    results.append(RuleResult("浪5偏短(失败特征)", True, False,
                              fib_score(r51, [0.236, 0.382, 0.500, 0.618], 0.1),
                              f"W5/W1={r51:.3f}"))
    return results
...
def check_leading_diagonal(waves: List[Wave], direction: Direction) -> List[RuleResult]:
    ...
    # 软规则
    results.append(RuleResult("浪5短于浪3", True, False,
                              1.0 if l5 < l3 else 0.3, f"W5={l5:.2f} W3={l3:.2f}"))

    results.append(RuleResult("浪3短于浪1(收敛)", True, False,
                              1.0 if l3 < l1 else 0.4, f"W3={l3:.2f} W1={l1:.2f}"))

    r21 = _ratio(w1, w2)
    results.append(RuleResult("浪2回撤深", True, False,
                              fib_score(r21, [0.618, 0.786], 0.08),
                              f"W2/W1={r21:.3f}"))
    return results
...
def check_ending_diagonal(waves: List[Wave], direction: Direction) -> List[RuleResult]:
    ...
    results.append(RuleResult("浪3非最短", hr_w3, True, 1.0 if hr_w3 else 0.0,
                              f"W1={l1:.2f} W3={l3:.2f} W5={l5:.2f}"))

    # 终结楔形标志: 浪4必须进入浪1区域
    hr4_overlap = not _above(w4.end.price, w1.end.price, d)
    results.append(RuleResult("浪4与浪1重叠(终结特征)", hr4_overlap, True,
                              1.0 if hr4_overlap else 0.0))
    ...
    # 软规则
    results.append(RuleResult("浪逐步缩短", True, False,
                              1.0 if l5 <= l3 <= l1 else (0.5 if l5 < l3 else 0.2)))

    l2, l4 = w2.length, w4.length
    results.append(RuleResult("浪4短于浪2", True, False,
                              1.0 if l4 < l2 else 0.3))
    return results
...
def check_zigzag(waves: List[Wave], direction: Direction) -> List[RuleResult]:
    ...
    results.append(RuleResult("B不超A起点", hr1, True, 1.0 if hr1 else 0.0,
                              f"B端={wb.end.price:.2f} A始={wa.start.price:.2f}"))

    # C浪超越A浪终点
    hr2 = _above(wc.end.price, wa.end.price, d)
    results.append(RuleResult("C超越A终点", hr2, True, 1.0 if hr2 else 0.0,
                              f"C端={wc.end.price:.2f} A端={wa.end.price:.2f}"))

    # HR3: Zigzag的B回撤不超过85% (区分Flat: Flat要求>=80-90%)
    rba = _ratio(wa, wb)
    hr3 = rba <= 0.85
    results.append(RuleResult("B回撤<85%(非Flat)", hr3, True, 1.0 if hr3 else 0.0,
                              f"B/A={rba:.3f}"))

    # 软规则
    results.append(RuleResult("B回撤A比率", True, False,
                              fib_score(rba, [0.382, 0.500, 0.618, 0.786], 0.08),
                              f"B/A={rba:.3f}"))

    rca = _ratio(wa, wc)
    results.append(RuleResult("C与A比率", True, False,
                              fib_score(rca, [0.618, 1.0, 1.272, 1.618], 0.1),
                              f"C/A={rca:.3f}"))

    results.append(RuleResult("B不过深", True, False,
                              1.0 if rba <= 0.786 else max(0, 1.0 - (rba - 0.786) / 0.2),
                              f"B回撤={rba:.1%}"))
    ...
    if da > 0 and db > 0 and dc > 0:
        ...
        results.append(RuleResult("时间比例(B慢于AC)", True, False,
...
def check_double_zigzag(waves: List[Wave], direction: Direction) -> List[RuleResult]:
    ...
    results.append(RuleResult("W与Y等长", True, False,
                              fib_score(ratio_wy, [0.618, 1.0, 1.618], 0.15),
                              f"Y/W={ratio_wy:.3f}"))

    x_len = w4.length
    x_ratio = x_len / w_len if w_len > 0 else 0
    results.append(RuleResult("X回撤比", True, False,
                              fib_score(x_ratio, [0.382, 0.500, 0.618], 0.1),
                              f"X/W={x_ratio:.3f}"))
    return results
...
def check_flat_regular(waves: List[Wave], direction: Direction) -> List[RuleResult]:
    ...
    hr1 = rba >= 0.80
    results.append(RuleResult("B回撤A>=80%", hr1, True, 1.0 if hr1 else 0.0,
                              f"B/A={rba:.3f}"))
    ...
    results.append(RuleResult("B回撤90-105%", True, False,
                              fib_score(rba, [0.90, 1.0], 0.08),
                              f"B/A={rba:.3f}"))

    # B回撤深度加分: B/A越高(>=85%)越像Flat，低于85%像Zigzag
    flat_ba_score = min(1.0, max(0.0, (rba - 0.80) / 0.15))  # 80%→0, 95%→1
    results.append(RuleResult("B回撤深(Flat特征)", True, False,
                              flat_ba_score,
                              f"B/A={rba:.3f} Flat匹配度={flat_ba_score:.2f}"))

    rca = _ratio(wa, wc)
    results.append(RuleResult("C约等于A", True, False,
                              fib_score(rca, [1.0, 1.272], 0.15),
                              f"C/A={rca:.3f}"))
    ...
    if da > 0 and db > 0 and dc > 0:
        ...
        results.append(RuleResult("时间均匀(Flat特征)", True, False,
                                  min(1.0, uniformity * 1.5),
                                  f"最短/最长={uniformity:.2f}"))
    return results
...
def check_flat_expanded(waves: List[Wave], direction: Direction) -> List[RuleResult]:
    ...
    results.append(RuleResult("B超A起点(扩展)", hr1, True, 1.0 if hr1 else 0.0,
                              f"B端={wb.end.price:.2f} A始={wa.start.price:.2f}"))
    ...
    rba = _ratio(wa, wb)
    results.append(RuleResult("B回撤100-138%", True, False,
                              fib_score(rba, [1.0, 1.236, 1.382], 0.1),
                              f"B/A={rba:.3f}"))

    rca = _ratio(wa, wc)
    results.append(RuleResult("C扩展比", True, False,
                              fib_score(rca, [1.0, 1.272, 1.618], 0.12),
                              f"C/A={rca:.3f}"))
...
def check_flat_running(waves: List[Wave], direction: Direction) -> List[RuleResult]:
    ...
    results.append(RuleResult("C未超A终点(顺势)", hr2, True, 1.0 if hr2 else 0.0,
                              f"C端={wc.end.price:.2f} A端={wa.end.price:.2f}"))

    rba = _ratio(wa, wb)
    results.append(RuleResult("B回撤比", True, False,
                              fib_score(rba, [1.0, 1.236, 1.382], 0.1),
                              f"B/A={rba:.3f}"))

    rca = _ratio(wa, wc)
    results.append(RuleResult("C偏短(顺势)", True, False,
                              fib_score(rca, [0.382, 0.500, 0.618], 0.1),
                              f"C/A={rca:.3f}"))
...
def check_triangle_contracting(waves: List[Wave], direction: Direction) -> List[RuleResult]:
    ...
    hr1 = shrink_count >= 3
    results.append(RuleResult("浪逐步缩短", hr1, True, 1.0 if hr1 else 0.0,
                              f"A={la:.2f} B={lb:.2f} C={lc:.2f} D={ld:.2f} E={le:.2f} "
                              f"缩短={shrink_count}/4"))
    ...
    results.append(RuleResult("极值点收敛", extremes_converge, True,
                              1.0 if extremes_converge else 0.0,
                              f"高点:{highs[0]:.1f}>={highs[2]:.1f}>={highs[4]:.1f} "
                              f"低点:{lows[1]:.1f}<={lows[3]:.1f}"))
    ...
    results.append(RuleResult("横盘结构(非趋势)", is_sideways, True,
                              1.0 if is_sideways else 0.0,
                              f"漂移/振幅={drift_ratio:.2f} 阈值<0.65"))

    # 软规则
    ratio_ca = lc / la if la > 0 else 0
    ratio_ec = le / lc if lc > 0 else 0
    results.append(RuleResult("C/A比率", True, False,
                              fib_score(ratio_ca, [0.618, 0.786], 0.1),
                              f"C/A={ratio_ca:.3f}"))
    results.append(RuleResult("E/C比率", True, False,
                              fib_score(ratio_ec, [0.618, 0.786], 0.1),
                              f"E/C={ratio_ec:.3f}"))
    return results
...
def check_triangle_expanding(waves: List[Wave], direction: Direction) -> List[RuleResult]:
    ...
    results.append(RuleResult("浪逐步放大", hr1, True, 1.0 if hr1 else 0.0,
                              f"A={la:.2f} C={lc:.2f} E={le:.2f}"))
    ...
    results.append(RuleResult("价格区间扩散", expanding, True,
                              1.0 if expanding else 0.0))
    ...
    results.append(RuleResult("横盘结构(非趋势)", is_sideways, True,
                              1.0 if is_sideways else 0.0,
                              f"漂移/振幅={drift_ratio:.2f}"))

    ratio_ca = lc / la if la > 0 else 0
    results.append(RuleResult("C/A扩展比", True, False,
                              fib_score(ratio_ca, [1.272, 1.618], 0.12),
                              f"C/A={ratio_ca:.3f}"))
    return results
...
def check_combination_wxy(waves: List[Wave], direction: Direction) -> List[RuleResult]:
    ...
    results.append(RuleResult("W与Y等长", True, False,
                              fib_score(ratio, [0.618, 1.0, 1.618], 0.15),
                              f"Y/W={ratio:.3f}"))

    # X回撤比
    x_len = w4.length
    x_ratio = x_len / w_len if w_len > 0 else 0
    results.append(RuleResult("X回撤W", True, False,
                              fib_score(x_ratio, [0.382, 0.500, 0.618], 0.1),
                              f"X/W={x_ratio:.3f}"))
    ...
    results.append(RuleResult("横向运动", True, False,
...
def check_volume_confirmation(waves: List[Wave], volumes: Optional[np.ndarray] = None) -> List[RuleResult]:
    ...
    if len(waves) == 5:
        ...
        results.append(RuleResult(
            "浪3成交量最大", True, False,
            1.0 if w3_largest else 0.3,
            f"V1={v1:.0f} V3={v3:.0f} V5={v5:.0f}"
        ))
    ...
    # === 3浪调整浪的成交量规则 ===
        ...
        results.append(RuleResult(
            "B浪成交量最低", True, False,
            0.8 if b_lowest else 0.4,
            f"VA={va:.0f} VB={vb:.0f} VC={vc:.0f}"
        ))
...
def validate_pattern(pattern_type: PatternType, waves: List[Wave],
                     direction: Direction) -> List[RuleResult]:
    """根据模式类型调用对应的验证函数"""
    checkers = {
