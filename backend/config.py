...
SWAP_LIMIT_ATR_OFFSET = 0.05          # v12.27.0: 0.15 → 0.05 (数据驱动)
                                      # 60 天数据: 0.15 撤单率 47.6% (10/21), 信号成交后 91% 胜率 +$905
                                      # 撤单 ≈ 错失同质信号利润, 浅回踩 0.05 平衡: 大部分能成交 + 仍享 maker 0.02% 费率
                                      # 限价 = 当前价 ± 0.05 × ATR (买等浅回踩, 卖等浅反弹)
